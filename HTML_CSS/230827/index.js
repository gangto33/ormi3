...
const user_name = users.map((user) => (`"_id": ${user._id},
"name": ${user.name},
"age": ${user.age}`))

console.log(user_name);

// user의 age가 27 이상인 사람의 데이터를 id만 추출해주세요.const user_name = users.map((user) => ["_id": user._id,

console.log(users.filter((user) => user.age >= 27).map((user) => `"_id": ${user._id}`))